...
def simulate(args, output=sys.stderr):
    ...
    rev_comp_ref_seqs = {
        name: reverse_complement(seq) for name, seq in ref_seqs.items()
    }
    frag_lengths = FragmentLengths(
        args.mean_frag_length, args.frag_length_stdev, output
    )
    adjust_depths(ref_seqs, ref_depths, ref_circular, frag_lengths, args)
    identities = Identities(
        args.mean_identity, args.identity_stdev, args.max_identity, output
    )
    ...
    while total_size < target_size:
        ...
        seq, quals, actual_identity, identity_by_qscores = sequence_fragment(
            fragment, target_identity, error_model, qscore_model
        )
        if len(seq) == 0:
            continue
...
def build_fragment(
    ...
    fragment = [
        get_start_adapter(start_adapt_rate, start_adapt_amount, args.start_adapter_seq)
    ]
    info = []
    frag_seq, frag_info = get_fragment(
        frag_lengths,
        ref_seqs,
        rev_comp_ref_seqs,
        ref_contigs,
    ...
    while random_chance(args.chimeras / 100):  # percentage to fraction
        ...
        info.append(','.join(frag_info))
    fragment.append(
        get_end_adapter(end_adapt_rate, end_adapt_amount, args.end_adapter_seq)
    )
    fragment = ''.join(fragment)
    fragment = add_glitches(
        fragment, args.glitch_rate, args.glitch_size, args.glitch_skip
    )
...
def sequence_fragment(fragment, target_identity, error_model):
    ...
    while True:
        ...
        for j in range(k_size):
            ...
            new_base = new_kmer[
                j
            ]  # can actually be more than one base, in cases of insertion
            ...
            if new_base != fragment_base and fragment_base == new_fragment_bases[i + j]:
                ...
                if change_count % settings.ALIGNMENT_INTERVAL == 0:
                    ...
                    if frag_len <= settings.ALIGNMENT_SIZE:
                        cigar = edlib.align(
                            fragment, ''.join(new_fragment_bases), task='path'
                        )['cigar']
...
def print_progress(count, bp, target, output):
    ...
    if percent > 100.0:
        percent = 100.0
    print(
        f'\rSimulating: {count:,} read{plural}  {bp:,} bp  {percent:.1f}%',
        file=output,
        flush=True,
        end='',
    )
...
def adjust_depths(ref_seqs, ref_depths, ref_circular, frag_lengths, args):
    ...
    for ref_name, ref_seq in ref_seqs.items():
        ...
        if not args.small_plasmid_bias and ref_circ:
            passing_total = sum(
                length for length in sampled_lengths if length <= ref_len
            )
            if passing_total == 0:
                sys.exit(
